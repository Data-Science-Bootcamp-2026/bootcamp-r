...
R is a programming language, but more than that, it is an interactive environment [@rfoundationwhat]. A typical R script is run line by line rather than compiled all at once. The [objects](06_objects.md) you create live in memory, allowing you to query, run, and tweak code continuously. 
...
Data scientists often use a mix of programmming languages. Each has its strengths. For example, R tends to be better for data processing and visualization, whereas Python tends to be better for machine learning.

But why write code at all? Well, think of how you would run a time series analysis on Microsoft Excel. The raw dataset you receive may contain typos, missingness, duplicates, and formatting errors. You may have to manually edit some cells. You may also receive multiple datasets to merge, rows to filter, and columns to create or remove or transform.
...
![R console](assets/images/r_console.png)
